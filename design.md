...
#### UGV — 真实 CARLA vehicle + BasicAgent
- 用 `vehicle.lincoln.mkz_2020` 或类似 dispatchable 模型 spawn
- `UGV_DISPATCH` 实现：
  ```python
  from agents.navigation.basic_agent import BasicAgent
  self.agent = BasicAgent(ugv_actor, target_speed=30)
  self.agent.set_destination(carla.Location(x, y, z))
  ```
- 每 tick post：`control = self.agent.run_step(); ugv_actor.apply_control(control)`
- `done()` 返回 True 时推送 `event_log: arrived`
- `UGV_RTL` 同理，destination = 起点
- 注意 BasicAgent 在 0.9.16 路径：`carla/PythonAPI/carla/agents/navigation/basic_agent.py`，需把该路径加入 PYTHONPATH 或拷贝到工程内
