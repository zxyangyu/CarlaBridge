> ⚠️ **ARCHIVED — 仅供历史追溯**
>
> 本文是 CarlaBridge v0.1 的初版架构设计。多处章节已被重构（refactor v0.3 + R11）推翻；
> 文内已对每段超时章节加 "**superseded by …**" 标注，但完整的当前设计已迁到仓库根的新 `design.md`。
>
> 当前权威文档：
> - `../../README.md` — 入门、运维、故障
> - `../../design.md` — 当前架构（精简版）
...
>
> 本文中已被 superseded 的章节（不要据此实现）：
> - §5.3 控制流（agent_ack / agent_reject 双向 emit） → 现 sio.call return-value + command_status 事件
> - §8 场景引擎（mock_agent_loop / SCRIPT 列表 / sim_time 触发剧本） → Bridge 完全时间无关
> - §9 Mock vs 真实 Agent（AgentLink 抽象 / `[agent] mode = mock|remote`） → `carlabridge/agent/` 目录已删
> - §10 协议与端口表 → 已加 `/scenario/{fire,reset,status}` 三端点
>
> 仍有效的部分（已并入新 `design.md`）：§3 线程模型、§4 模块分解、§7 相机绑定模型、§11 技术栈、§14 失败处理、§17 性能预算。

---

# CarlaBridge 架构设计（design.md） — v0.1 历史版
